...
typedef struct BulkWork {
    ...
    uint64_t (*get_widget_life)(uint64_t);
    uint64_t (*get_unit_max_hp)(uint64_t);
...
_Static_assert(sizeof(BulkWork) == 624, "BulkWork ABI");
__declspec(dllexport) const uint32_t bulk_batch_abi[3] = {0x24268023u,216u,624u};
...
static uint32_t BridgeBulkGroup(BulkWork *w, uint64_t player, uint32_t *changed) {
    ...
    while ((unit = w->first_of_group(group)) != 0) {
        ...
        if (w->action == BULK_HEAL_LOCAL) {
            uint64_t hp_bits = w->get_unit_max_hp(unit);
            union { uint64_t bits; float value; } hp;
            hp.bits = hp_bits;
            if (!(hp.value == hp.value) || hp.value < 0.0f || hp.value > 1000000000.0f) {
                w->destroy_group(group); return 0;
            }
            w->set_widget_life(unit, &hp.value);
...
__declspec(dllexport) uint64_t BridgeBulkQuery(void) {
    ...
    if (!w || w->expected_tls != g_dispatch->tls_value || w->action < BULK_HEAL_LOCAL ||
        w->action > BULK_PEACE_MODE || w->value > 1 || !w->create_group ||
    ...
    if (!changed) { w->error = 144; return 0; }
    w->changed = changed;
    w->completed = 1;
    return changed;
}
