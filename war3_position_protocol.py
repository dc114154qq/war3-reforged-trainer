...
ROW_SIZE = 32
MAX_UNITS = 24
ABI = struct.pack("<3I", 0x2426801D, 216, WORK_SIZE)
SIGNATURES = (
    ("SetUnitX", "(Hunit;R)V"),
    ("SetUnitY", "(Hunit;R)V"),
    ("GetUnitX", "(Hunit;)R"),
...
def validate_work(payload):
    if len(payload) != WORK_SIZE:
        raise ValueError("PositionWork must contain exactly 1312 bytes")
    validate_selection_work(payload[:480])
    values = struct.unpack_from("<5Q6I", payload, 480)
    handlers, tls = values[:4], values[4]
    x_bits, y_bits, changed, error, completed, reserved = values[5:]
    if len(set(handlers)) != 4 or any(not 0x10000 <= address < 0x800000000000 for address in handlers):
...
def decode_work(payload, expected_count):
    ...
    for index, source in enumerate(selection["rows"]):
        ...
        if (unit != source["handle"] or status != 1 or reserved_row
                ...
                or abs(actual_x_value - target_x) > 0.01
                or abs(actual_y_value - target_y) > 0.01):
