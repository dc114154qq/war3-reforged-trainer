...
ELEPHANT_HOTKEY_SPECS = (
    GlobalHotkeySpec("backup_read_unit", "Ctrl+F11  读取所有选中单位", MOD_CONTROL, VK_F11),
            ...
            for scan_start, scan_end in scan_ranges:
                for region in sorted(regions, key=lambda item: item.base):
                    ...
                    for block_start, blob in self._iter_readable_blocks_win10(
                        ...
                        first_record = max(record_scan_start, (block_start + 7) & ~7)
                        ...
                        for record in range(first_record, block_end - 24, 8):
                            ...
                            if name not in missing:
                                name = self._decode_native_name_from_record(
                                    pm,
                                    record,
                                    external_names=external_names,
                                )
...
class BackupReadWar3Trainer(War3Trainer):
        ...
        *,
        ...
        try:
            if ptr == record + 0x18 and (capacity & 0xFF) >= size:
                ...
                name = external_names[ptr]
                return name if len(name) == size else None
            elif self._sane_heap_ptr(ptr) and size <= capacity < 0x1000:
                data = pm.read(ptr, int(size))
            else:
                return None
        except OSError:
    ...
    def _direct_selected_context(self) -> tuple[UnitCandidate, int]:
        ...
        with self._process_memory() as pm:
            safe_pm = self._require_win10_memory(pm)
            snapshot = self._selected_candidates_snapshot(safe_pm)
        if not snapshot:
            raise RuntimeError("游戏当前没有可操作的选中单位")
        # Keep the candidate and JASS handle paired from one backup snapshot.
        return snapshot[0]
...
def run_gui() -> None:
    ...
    def elephant_trainer() -> War3Trainer:
        ...
        t = trainer()
        _panel, candidate, _fields = t.read_selected_unit_fields_win10()
        identity = unit_identity(candidate)
        return t.trainer_for_read_source(identity, True)
    ...
    def set_advanced_unit_field() -> str:
        ...
        if target_identity is None:
            _panel, candidate, _fields = t.read_selected_unit_fields_win10()
            target_identity = unit_identity(candidate)
