...
从 GitHub Release 下载 `War3ReforgedTrainer-v0.25.exe`，直接双击运行即可。Release 页面里它的显示标签是 `魔兽争霸3重制版修改器.exe`。这是 PyInstaller 打包的单文件 GUI 程序，不会额外弹出终端窗口，也不需要安装 Python。
...
- 英雄技能：读取并替换已学技能的 rawcode。写入时会同步英雄技能配置和运行时 ability 实例，并刷新命令卡；重复 rawcode、空技能槽、找不到存活运行时模板的目标技能会被拒绝写入。

## 当前限制

- 技能替换不是凭空创建技能。目标技能必须能在当前地图中的存活单位上找到运行时模板；找不到模板、只有死亡单位模板、当前单位已有重复 rawcode 或原技能槽无法唯一定位时，修改器会拒绝写入。
...
- 技能修改也走字段表路径：选择 `skillN_name`，填写目标技能 rawcode 后点击 `写入字段`。如果目标 rawcode 已存在于当前单位，或地图里找不到存活单位持有该技能作为模板，写入会失败并给出原因。
