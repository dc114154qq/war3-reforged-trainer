...
SIGNATURES=(('UnitAddItemById','(Hunit;I)Hitem;'),('UnitItemInSlot','(Hunit;I)Hitem;'),
...
def build_work(entries,tls,action=0,rawcode=0,charges=-1):
    ...
    for name,sig in SIGNATURES:
        e=entries.get(name)
        if e is None or e.name!=name or e.signature!=sig:raise ValueError('Item native signature differs: '+name)
        pointers.append(e.handler)
    payload=select_work(entries)+struct.pack('<9Q2Ii5I',*pointers,tls,rawcode,action,charges,0,0,0,0,0)+bytes(5376)
    validate_work(payload);return payload

def validate_work(payload):
    if len(payload)!=WORK_SIZE:raise ValueError('Item batch work must contain 5960 bytes')
    validate_selection(payload[:480]);v=struct.unpack_from('<9Q2Ii5I',payload,480)
...
def decode_work(payload,count):
    ...
    rawcode,action,target,changed,error,completed,skipped,reserved=struct.unpack_from('<2Ii5I',payload,552)
