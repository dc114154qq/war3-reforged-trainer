...
$verify='import pefile,struct,sys; p=pefile.PE(sys.argv[1]); e={s.name:s.address for s in p.DIRECTORY_ENTRY_EXPORT.symbols}; assert p.FILE_HEADER.Machine==0x8664; assert p.get_data(e[b"bridge_abi"],12)==struct.pack("<3I",0x24268010,216,608); assert all(n in e for n in (b"BridgeInstall",b"BridgeUninstall",b"BridgeHeroQuery",b"BridgeAbilityQuery",b"BridgeItemQuery")); assert p.get_data(e[b"ability_batch_abi"],12)==struct.pack("<3I",0x24268011,216,832); assert p.get_data(e[b"item_batch_abi"],12)==struct.pack("<3I",0x24268014,216,5960); assert (b"BridgeTestRun" in e)==(sys.argv[2]=="True"); print("x64 bridge ABI verified")'
