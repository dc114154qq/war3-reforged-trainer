...
审计日期：2026-09-06。起始提交：`946d24a`，当前提交：`dd44267` 加本次未发布修改，分支：`codex/v1.0.19-followup`。
...
| 英雄、非英雄、物品、数量、技能字段完整 | C `War3PersistentSnapshot` 有基础值、英雄属性、六个物品槽及数量；技能数组固定 48 项，枚举最多 256 索引；当前协议 25 在超过 48 项时返回 `ERROR_MORE_DATA`，Python 拒绝不完整结果 | **未完成**。已消除静默截断，但仍没有可变技能结果；应扩展协议并定义枚举结束/异常语义，验证边界和大量技能。不能把“拒绝不完整”宣称为“完整读取” |
