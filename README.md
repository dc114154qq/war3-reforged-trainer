...
从 GitHub Release 下载 `War3ReforgedTrainer-v1.0.17-Normal.exe`，直接双击运行并允许 Windows UAC 管理员权限确认。这是 PyInstaller 打包的单文件 GUI 程序，不会额外弹出终端窗口，也不需要安装 Python。管理员权限和按需启用的 `SeDebugPrivilege` 用于访问受到进程权限限制的 Warcraft III，避免 `WinError 5: 拒绝访问`。
