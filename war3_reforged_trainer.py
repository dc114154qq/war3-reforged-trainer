...
def run_gui() -> None:
    ...
    id_catalog_statuses = {
        kind: tk.StringVar(value=f"记录数：{CATALOG_COUNTS[kind]}")
        for kind in ("item", "ability", "unit")
    }
    ...
    def copy_selected_catalog_id(kind: str) -> None:
        selected = id_catalog_trees[kind].selection()
        if not selected:
            messagebox.showerror("错误", "请先在 ID 表格里选择一项", parent=root)
            return
    ...
    def build_id_catalog_tab(kind: str, title: str) -> None:
        ...
        ttk.Label(toolbar, text="搜索 ID、中文名、英文名或类别").grid(
            row=0,
            column=0,
            sticky="w",
        )
        ...
        ttk.Label(
            tab,
            text=(
                "双击表格行可复制 ID。数据来源：W3x2LNI zhCN-1.32.8 与 "
                "war3-objectdata 英文社区快照；自定义地图对象不在通用目录内。"
            ),
            anchor="w",
            justify="left",
            wraplength=1080,
