...
def test_bootstrap_uses_one_dll_command_and_discards_previous_process_records():
    ...
    assert trainer.persistent_native_init() == len(trainer.PERSISTENT_NATIVE_NAMES)
...
def test_additional_natives_use_profile_indexes_and_atomic_result_publication():
    ...
    result = trainer._discover_native_handlers_near_table(Mock(), ['SetUnitScale'])
    assert result['SetUnitScale'].handler_address == 0x200000
    trainer._run_native_helper_ops.assert_called_once_with(0, ((140, NATIVE_INDEX['SetUnitScale'], 0, PROFILE_ID, 0),))
