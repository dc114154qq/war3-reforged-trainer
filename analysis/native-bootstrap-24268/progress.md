...
随后对 loader 完整运行时 `.text` 做了同类原始字节检查，结果发现 16 个引用落在三个游戏上下文锚点及其相邻字段（包括 `0x1af98da` / `0x1af9bbd` 读取两个游戏基址字段，以及 `0x1af9d74` 附近的上下文结构引用）。其中 `0x1af9700` 附近是混淆分派/解码函数，会根据状态值生成分支结果；它是后续离线分析的有效入口，但不能直接调用。此前“loader 代码没有直接引用上下文”的表述已修正。
